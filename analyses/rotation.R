...
df <- read_csv("results/sweeps/rotation/resnet_rotation_no_gaussian.csv") %>% 
  bind_rows(read_csv("results/sweeps/rotation/ft_transformer_rotation.csv")) %>% 
  bind_rows(read_csv("results/sweeps/rotation/gpt_rotation.csv")) %>% 
  bind_rows(read_csv("results/sweeps/rotation/rf_rotation.csv")) %>% 
  filter(transform__0__max_rel_decrease == 0.015) %>% 
  bind_rows(read_csv("results/sweeps/sweep_resnet.csv") %>% 
              mutate(transform__1__method_name = "no_rotation")) %>% 
              #slice_head(n=2000)) %>% 
  bind_rows(read_csv("results/sweeps/sweep_ft_transformer.csv") %>% 
              mutate(transform__1__method_name = "no_rotation")) %>% 
             # slice_head(n=2000)) %>% 
  bind_rows(read_csv("results/sweeps/sweep_gbt.csv") %>% 
              mutate(transform__1__method_name = "no_rotation")) %>% 
              #slice_head(n=2000)) %>% 
...
df_normalized <- df %>% 
  ...
  select(model_name, data__keyword, mean_val_score, mean_test_score, mean_time, transform__1__method_name)
...
for (i in 1:N_SHUFFLE) {
  new_df <- df_normalized %>% 
    ...
    group_by(model_name, data__keyword, transform__1__method_name) %>% 
  ...
  res <- res %>%  bind_rows(new_df %>% 
                              ...
                              group_by(model_name, data__keyword, transform__1__method_name, mean_val_score) %>% 
                              ...
                              ungroup() %>% 
                              complete(model_name, data__keyword, transform__1__method_name, random_rank) %>%
                              #select(model_name, data__keyword, log_cum_time_factor,log_cum_time, mean_val_score,best_val_score_so_far, mean_test_score)) %>% 
                              group_by(model_name, transform__1__method_name, data__keyword) %>% 
                              ...
                              #select(model_name, data__keyword, log_cum_time_factor,log_cum_time, mean_val_score,best_val_score_so_far, mean_test_score) %>% 
                              ungroup() %>% 
                              group_by(model_name, transform__1__method_name, random_rank) %>% 
                              # mean on datasets
...
ggplot() +
  ...
  facet_wrap(~transform__1__method_name)+
...
ggsave("analyses/plots/rotation.jpg", width=16, height=9)
