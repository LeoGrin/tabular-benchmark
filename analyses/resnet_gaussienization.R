...
df <- read_csv("results/sweeps/resnet_gaussienization.csv") %>% 
  bind_rows(read_csv("results/sweeps/rf_gaussienization.csv"))
...
df_normalized <- df %>% 
  ...
  select(model_name, data__keyword, mean_val_score, mean_test_score, mean_time, transform__0__type)
...
for (i in 1:N_SHUFFLE) {
  new_df <- df_normalized %>% 
    add_column(random_number = runif(nrow(.))) %>% 
    group_by(model_name, data__keyword, transform__0__type) %>% 
  ...
  res <- res %>%  bind_rows(new_df %>% 
                              ...
                              #filter(mean_val_score == best_val_score_so_far) %>% 
                              # Only keep the first time we beat valid score
                              group_by(model_name, data__keyword, transform__0__type, mean_val_score) %>% 
                              #filter(random_rank == min(random_rank)) %>%
                              mutate(mean_test_score = if_else(random_rank == min(random_rank), mean_test_score, NA_real_)) %>% 
                              ungroup(mean_val_score) %>% 
                              #for every dataset, prolong the curve by the last best value when there is no value left (for long time)
                              ungroup() %>% 
                              complete(model_name, data__keyword, transform__0__type, random_rank) %>%
                              #select(model_name, data__keyword, log_cum_time_factor,log_cum_time, mean_val_score,best_val_score_so_far, mean_test_score)) %>% 
                              group_by(model_name, transform__0__type, data__keyword) %>% 
                              #mutate(mean_val_score = na.locf(mean_val_score, na.rm=F)) %>% 
                              mutate(mean_test_score = na.locf(mean_test_score, na.rm=F))%>% 
                              #filter(model_name == "Resnet") %>% 
                              #select(model_name, data__keyword, log_cum_time_factor,log_cum_time, mean_val_score,best_val_score_so_far, mean_test_score) %>% 
                              ungroup() %>% 
                              group_by(model_name, transform__0__type, random_rank) %>% 
                              # mean on datasets
                              # mean will be NA if at least one score for one dataset is missing for a given time bin
...
ggplot() +
  #geom_hline(aes(yintercept=mean_test_score, color=model_name), data=res_ %>% filter(random_rank == 0), linetype="dotted", size=1.5) +
  geom_line(aes(x = random_rank, y = mean_test_score, color = transform__0__type), data=res_, size=2) +
  geom_dl(aes(label = transform__0__type, x=random_rank, y=mean_test_score, color=transform__0__type), method = list(dl.combine("smart.grid"), cex=1.3), data=res_)  +
  geom_ribbon(aes(x=random_rank, ymin = min_test_score, ymax=max_test_score, fill=transform__0__type), data=res_, alpha=0.3) +
...
ggsave("analyses/plots/gaussienization.jpg", width=16, height=9)
