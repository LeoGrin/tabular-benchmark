...
df_normalized <- df %>% 
  ...
  select(model_name, data__keyword, mean_val_score, mean_test_score, mean_time, transform__0__multiplier)
...
for (i in 1:N_SHUFFLE) {
  new_df <- df_normalized %>% 
    ...
    group_by(model_name, data__keyword, transform__0__multiplier) %>% 
  ...
  res <- res %>%  bind_rows(new_df %>% 
                              ...
                              group_by(model_name, data__keyword, transform__0__multiplier, mean_val_score) %>% 
                              ...
                              ungroup() %>% 
                              complete(model_name, data__keyword, transform__0__multiplier, random_rank) %>%
                              #select(model_name, data__keyword, log_cum_time_factor,log_cum_time, mean_val_score,best_val_score_so_far, mean_test_score)) %>% 
                              group_by(model_name, transform__0__multiplier, data__keyword) %>% 
                              ...
                              #select(model_name, data__keyword, log_cum_time_factor,log_cum_time, mean_val_score,best_val_score_so_far, mean_test_score) %>% 
                              ungroup() %>% 
                              group_by(model_name, transform__0__multiplier, random_rank) %>% 
                              # mean on datasets
...
ggplot() +
  ...
  geom_line(aes(x = random_rank, y = mean_test_score, color = percentage_added), data=res_, size=2) +
  geom_dl(aes(label = percentage_added, x=random_rank, y=mean_test_score, color=percentage_added), method = list(dl.combine("smart.grid"), cex=1.3), data=res_)  +
  geom_ribbon(aes(x=random_rank, ymin = min_test_score, ymax=max_test_score, fill=percentage_added), data=res_, alpha=0.3) +
...
ggsave("analyses/plots/added_features.jpg", width=16, height=9)
